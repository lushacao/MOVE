...
class move:
    ...
    def log2_transform(self, figsize= (10,10)):
        ...
        #drop all NAs proteins (not sure why this situation would happen but need to do more investigation)
        self._log = self._log[self._log.count(axis=1) > 0]

        # visualize 
        plt.figure(figsize=figsize)
        sns.boxplot(data=self._log, orient="h",color = 'lightsteelblue')
    ...
    def median_normalize(self, figsize= (10,10)):
        ...
        # visualize 
        plt.figure(figsize=figsize)
        sns.boxplot(data=self._median, orient="h",color = 'lightsteelblue')
        plt.title('Normalized Intensity Distribution',size ='large')
    ...
    def plot_common_proteins(self, df_common, figsize = (15,8)):
        ...
        cat_pal = sns.color_palette("colorblind",self.samples['Sample type'].nunique())
        ...
        # plot the figure
        plt.figure(figsize=figsize)
        ax = pro_count.plot(kind='barh',color=colors)
        ax.set_xlabel('Protein Count',size ='large')
